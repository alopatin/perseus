...
_.extend(Expr.prototype, {
    ...
    recurse: function(method) {
        return this.construct(_.invoke(this.args(), method));
    },
    ...
    // TODO(alex): generalize to a more powerful .has(func/node/nodelist) ?
    // return whether this expression contains any Trig or Log nodes
    hasTrigLog: function() {
        return _.any(_.invoke(this.args(), "hasTrigLog"));
    },
...
_.extend(Add.prototype, {
    ...
    isNegative: function() {
        return _.all(_.invoke(this.collect().terms, "isNegative"));
    }
});
...
_.extend(Mul.prototype, {
    ...
    collect: function() {
        ...
        // [[Expr base, Expr exp]]
        var summed = _.map(grouped, function(pairs) {
            var base = pairs[0][0];
            var sum = new Add(_.zip.apply(_, pairs)[1]);
            var exp = sum.collect();
            return [base, exp];
        ...
        if (trigs.length > 1) {
            // {(Trig base).arg.print(): [[Trig base, Expr exp]]}
    ...
    factorIn: function(hint) {
        ...
        if (fold) {
            // e.g. - x*2*3 -> x*-2*3
            return this.replace(numbers[0], numbers[0].negate().hint(hint));
        } else {
            // e.g. - x*y -> -1*x*y
            // e.g. - x*-2 -> -1*x*-2
    ...
    factorOut: function() {
        ...
        var terms = _.compact(_.map(this.terms, function(term, i, list) {
            if (!factored && term instanceof Num && term.hints.divide) {
                factored = true;
                // if (term.eval() === -1) return null;

                // var negated = term.negate();
                // negated.hints = _.clone(term.hints);
                // negated.divide = false;
                // return negated;
...
_.extend(Mul, {
    ...
    handleNegative: function(expr, hint) {
        if (expr instanceof Num && expr.n > 0) {
            ...
            negated.hints = _.clone(expr.hints);

            return negated.hint(hint);
    ...
    fold: function(expr) {
        if (expr instanceof Mul) {
            ...
            if (triglog) {
                ...
                if (last.hints.parens || last.hasTrigLog()) {
...
_.extend(Log.prototype, {
    ...
    expand: function() {
        ...
        if (log.power instanceof Mul) {  // might want behind super-simplify() flag
            ...
            return new Mul(log.power.exp, new Log(log.base, log.power.base));
        } else {
            return log;
        }
    },
    ...
    hasTrigLog: function() { return true; },
...
_.extend(Trig.prototype, {
    ...
    recurse: function(method) {
        return new this.func(this.type, this.arg[method]());
    },
...
_.extend(Atom.prototype, {
    ...
    needsExplicitMul: function() { return false; },
    hasTrigLog: function() { return false; },
    findGCD: function(factor) {
...
_.extend(Var.prototype, {
    ...
    getVars: function() { return [this.print()]; },
...
Num.Sqrt = new Rational(1, 2).hint("root");
